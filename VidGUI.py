...
def get_input(event):
	...
	try:
		...
		get_input.write1= customtkinter.CTkLabel(root, text=good, font=("Helvetica", 12), text_color="green")
		get_input.write1.grid(row=2, column=0, padx=(5, 0), sticky="w")
	except Exception as e:
		# Handle exceptions and display an error message
		get_input.write1 = customtkinter.CTkLabel(root, text=bad, font=("Helvetica", 12), text_color="red")
		get_input.write1.grid(row=2, column=0, padx=(5, 0), sticky="w")
...
write = customtkinter.CTkLabel(root, text=Ins, font=("Helvetica", 15), text_color="#00eeff", padx=10)
write.grid(row=0, column=0)


## Input bar for user's input
entry = customtkinter.CTkEntry(root, width=350)
entry.configure(corner_radius=5, placeholder_text="Paste the youtube video URL", placeholder_text_color="grey")
...
download_button.configure(text_color="white", fg_color="red", corner_radius=5, hover=True, width=80, hover_color="#ff5057")
...
vidType = customtkinter.CTkLabel(root, text=vidText, text_color="white")

## Positioning download options
## Styling
vidType.grid(row=3, column=0, sticky="w", padx=(5, 0))
video_option.configure(radiobutton_width=10, radiobutton_height=10, fg_color="#00eeff")
audio_option.configure(radiobutton_width=10, radiobutton_height=10, fg_color="#00eeff")
...
vidQual = customtkinter.CTkLabel(root, text=qualText, text_color="white")

## Positioning download quality options
## Styling
vidQual.grid(row=4, column=0, sticky="w", padx=(5, 0))
quality1.configure(radiobutton_width=10, radiobutton_height=10, fg_color="#00eeff")
quality2.configure(radiobutton_width=10, radiobutton_height=10, fg_color="#00eeff")
